...
* If x <= T[p] and hence less than or equal to all the elements of T[p..r], then it returns the index p.
*  If x > T[p], then it returns the largest index q in the range p < q  < r + 1 such that T[q  - 1] < x. Here is the pseudocode:
